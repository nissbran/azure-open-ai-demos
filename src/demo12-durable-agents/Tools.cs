...
{
    internal sealed class PartSupplierSearch()
    {
        private readonly SearchClient _searchClient;

        public PartSupplierSearch(IConfiguration configuration) : this()
        {
        ...
        [Description(
            "Search supplier parts. It does not contain what parts goes where. The data contains part number, part name, part type, category, supplier, unit cost, lead time in weeks, country of origin, warranty period in months, and stock status.")]
        public async Task<List<SupplierPartSearchResult>> SearchForSupplierParts(
            [Description("Seach query for supplier parts")]
            string searchQuery)
        {
            try
            {
